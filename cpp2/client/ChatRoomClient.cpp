...
int main(int argc, char* argv[]) {
  ...
  try {
    ...
    *sendRequest.sender_ref() = getenv("USER");
    LOG(INFO) << "Sending...";
    client->future_sendMessage(sendRequest).then([](folly::Try<folly::Unit>&&){
      LOG(INFO) << "Sent!";}
    ).get();
    ...
    client->future_getMessages(getRequest).then([](folly::Try<example::chatroom::GetMessagesResponse>&& response){LOG(INFO) << "Got!";
      if (response.hasException()){
        LOG(ERROR) << "Get messages failed: " << response.exception().get_exception()->what();
        return;
      }
